...
from modules.jira_operations import get_project_keys,create_jira_issue_ticket_template,save_jira_project_key,get_jira_issue_type_project_key,get_children_issues_ticket_template,get_jira_issue_type_project_key_with_displayname,display_issue_summaries,authenticate
...
def main():
    ...
    if st.session_state['api_password'] == '':
        ...
        try:
            jira=authenticate(JIRA_URL,st.session_state['api_username'],st.session_state['api_password'])
...
################################Select and edit the ticket template####################################################################
...
################################Select the board and project and other details####################################################################
            
            # Step 1: Get Board Keys from Jira
...
################################ Create the ticket ####################################################################
            ...
            if st.button("Create Ticket"):
                ...
                if not jira:
                    ...
                    try:
                        final_summary=f'[{board_key}][{project_name}] {final_summary}'
...
#  if ticket is created for CoE board -> no assign to CoE status
#  clarify how ML tickets are handled
# if ticket is for annotation - create a new status ? or simply just create it on customer board
