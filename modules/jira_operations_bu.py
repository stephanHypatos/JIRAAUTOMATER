...
from modules.config import JIRA_PROJECT, JIRA_ACCOUNT_ISSUE_TYPE,JIRA_PROJECT_ISSUE_TYPE,JIRA_EPIC_ISSUE_TYPE, JIRA_TASK_ISSUE_TYPE, JIRA_SUBTASK_ISSUE_TYPE,JIRA_URL,EXCEL_FILE_PATH
...
def get_issue_key(jira, summary):
    ...
    issues = jira.search_issues(f'project={JIRA_PROJECT} AND summary~"{summary}"', maxResults=1)
    ...
    for index, row in excel_data.iterrows():
        ...
        if link1_key and link2_key:
            ...
            if link1_key_normalized:
                ...
                print(f"Link1 issue '{link1_key}' not found. Skipping link creation.")
            ...
            if link1_key_normalized:
                ...
                for block_key in blocks_list:
                    ...
                    if link2_key_normalized:
                        ...
                        print(f"Link2 issue '{block_key}' not found. Skipping link creation.")
            else:
                print(f"Link1 issue '{link1_key}' not found. Skipping link creation.")
    ...
    for index, row in excel_data.iterrows():
        ...
        if issue_type == JIRA_EPIC_ISSUE_TYPE:
            ...
            if parent_key is not None:
                ...
                parent_issue = jira.search_issues(f'project={JIRA_PROJECT} AND summary~"{parent_key}"', maxResults=1)
                ...
                if parent_issue:
                    ...
                    print(f"Created Jira task: {task_issue.key} - Summary: {task_issue.fields.summary}, Linked to parent: {parent_key}")

                else:
                    print(f"Parent issue '{parent_key}' not found. Skipping task creation.")
            else:
                ...
                task_issue = jira.create_issue(fields=issue_dict)
                print(f"Created Jira task: {task_issue.key} - Summary: {task_issue.fields.summary}")
            ...
            if parent_key is not None:
                ...
                if parent_issue:
                    ...
                    print(f"Created Jira subtask: {subtask_issue.key} - Summary: {subtask_issue.fields.summary}, Linked to parent: {parent_key}")
                else:
                    print(f"Parent issue '{parent_key}' not found. Skipping subtask creation.")
            else:
                print("A Sub-task must have a parent. Skipping subtask creation.")
...
def get_issues_from_jira(jira):
    ...
    issues = jira.search_issues(f'project={JIRA_PROJECT}', maxResults=None)
        ...
        if issue_key_checked and new_status:
            try:
                ...
                if (
                    ...
                    if existing_issue.fields.customfield_10127:
                        ...
                        print('does not exidts') 
                        print(existing_issue.fields.customfield_10127)
                        print(row['ExternalAssignee'])
                        existing_issue.update(
                            summary=row['Summary'],
                            #description=row['Description'],
                            customfield_10015=normalize_date(row['StartDate']),
                    ...
                    print(f"Updated Jira issue: {issue_key} - Summary: {row['Summary']}")
            except Exception as e:
                print(f"Error updating issue {issue_key} in Jira: {e}")
            ...
            if parent_key is not None:
                ...
                #TO DO IMPLEMENT HANDLING FOR EPICs
                    ...
                    print(f"Parent issue '{parent_key}' not found. Tasks can only be created if ParentName is provided. Skipping task creation.")
            ...
            print(f"Created Jira issue: {new_issue.key} - Summary: {new_issue.fields.summary}")
...
def transition_issue(jira, issue_key, new_status):
    ...
    if transition_id:
        ...
        print(f"Issue {issue_key} transitioned to status: {new_status}")
    else:
        print(f"Transition to status {new_status} failed. Transition from current status to {new_status} not allowed.")
...
def has_cf(jira):
    ...
    try:
        ...
        fields = jira.search_issues(f'project={JIRA_PROJECT} AND cf[10127] is not EMPTY', maxResults=1)
        if fields: 
            print(f'CustomField is available in {JIRA_PROJECT}')
            return True
        print(f'CustomField is not available in {JIRA_PROJECT}')
        return False
...
## CAN BE DELETED
#def create_Start_and_End_date(excel_data,project_startdate, duration_dict, dependency_dict):
    # Iterate through rows and create Jira issues and subtasks
    #for index, row in excel_data.iterrows():
        #summary = row['Summary']
        #issue_type = row['IssueType']
        ...
        #start_date, due_date = compute_dates(summary, issue_type, parent_key, project_startdate, dependency_dict, duration_dict,excel_data)
        #st.write(f'Startdate: {start_date} and Due_date: {due_date}')
            ...
            for link in row['IssueLinks']:
                if link['LinkType'] == 'blocks':
                    ...
                    if row['DueDate'] != blocking_issue['StartDate']:
                        print(row['IssueKey'])
                        #print(row['DueDate'])
                        print(blocking_issue['StartDate'])
                        print(blocking_issue['DueDate'])
...
def update_dates_in_jira(jira, updated_task_df):
    # Your code to update the start_date and due_date (or end_date) in Jira
    pass


def update_child_subtasks(jira, parent_key, start_date, end_date):
    # Your code to get and update child sub-tasks in Jira
    pass

def update_parent_epics(jira, task_df):
    # Your code to update parent epics in Jira
    pass
